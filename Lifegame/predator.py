...
class Predator:
    ...
    def move(self, creatures):
        ...
        if prey:
            dx, dy = self._vector_towards(prey.x, prey.y)
        else:
    ...
    def try_eat(self, creatures):
        for c in creatures:
            if math.hypot(self.x - c.x, self.y - c.y) < self.DIAM:
                creatures.remove(c)
                self.energy = min(MAX_ENERGY, self.energy + c.energy)
                self._choose_new_target()
                return True
        return False
    ...
    def maybe_divide(self, predators):
        if self.energy >= MAX_ENERGY * 0.8:
            ...
            predators.append(
                Predator(cx, cy, MAX_ENERGY * 0.25, child_speed, child_smell)
            )
            self._choose_new_target()
    # ----------------------------------
    ...
    def draw(self, surf, scale_x, scale_y):
        ...
        pygame.draw.circle(surf, (0,   0,   0),   (sx, sy),
