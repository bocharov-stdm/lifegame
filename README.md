...
У травоядных есть геном — размер, скорость, зрение, порог размножения, доля
энергии потомку и вертикальный слой обитания (`min_y`, `max_y`), у хищников —
скорость и зрение. Гены мутируют при делении, отбор идёт сам собой. У обоих
видов есть и **мутагенность** — множитель на разброс мутаций потомка: всех
генов, включая её саму, и шанса сменить стратегию. Геном
каждого вида — таблица генов, а поведение — стратегия, которая тоже
наследуется: так в мир добавляются новые гены и новые повадки.

Слой мягкий: травоядное держится своей глубины, но за видимой едой выходит из
слоя и потом возвращается. Стратегий у каждого вида две:

- травоядные: **стандартный** (бежит от хищника, идёт к еде, бродит) и
  **затаившийся** — то же, но пока не видит еды, бродит втрое медленнее и тратит
  меньше;
- хищники: **стандартный** (голодный гонится за добычей, вблизи — рывком) и
  **засадник** — всё время бродит медленно и дёшево, бросается рывком только на
  близкую добычу.

Потомок изредка получает другую стратегию, и отбор решает, какая выживет.

По умолчанию мир — только растения и травоядные: хищников нет ни в игре, ни
в отчёте, ни в эталоне баланса. Выпустить их можно явно — галочкой «Хищники» в
«Новом мире» или флагом `--predators N`.

**Каннибализм** — правило лаборатории: травоядное съедает сородича, который
мельче его в 2.5 раза (отношение настраивается), если тот оказался вплотную.
Никто никого не ищет — едят тех, кто рядом. В игре оно включено по умолчанию, в
отчёте — `--rule cannibalism=1` (отношение — `--rule cannibal_ratio=2`).
...
- **Новый мир**: масштаб (от «Как раньше» ×1 до «Планеты» ×1000 или свой, до
  ...
  полоса), сид, стартовые численности, рост растений, доли затаившихся и
  засадников на старте; вкладка «Еда» — где растут растения, с картинкой;
...
- **В игре**: пауза (Пробел), тик (→), скорость (+/−), колесо — зум к курсору,
  ...
  партии (и на паузе), I — окно «Статистика»: сытость и голод, где живут
  травоядные и где растёт еда (глубина во времени и по полосам), геном
  хищников; инструмент «Область» — протянуть прямоугольник по миру и увидеть
  средний геном тех, кто внутри, рядом со средним по миру. Подсадка травоядных
  (и хищников, если они в партии) кликом, «Заново» — та же партия с начала,
  Esc — меню.
...
cargo run -p life-report --release -- --veg-mix 1 1 --pred-mix 1 1  # стратегии поровну на старте
...
| `life-core/src/vegetarian/`, `predator/`, `plant.rs` | существа: фенотип, стратегии поведения |
| `life-core/src/genome/` | таблицы генов обоих видов и мутация |
...
Медленный ход (у затаившегося и засадника) стоит дешевле: за скорость платится
по фактическому шагу, по тому же закону `скорость ** 2`, поэтому треть скорости
обходится в девятую часть. До мягкого слоя и новых стратегий 4 мира из 12
вымирали за 20 000 тиков; с ними — ни одного. С мутагенностью снова вымирает
половина миров с хищниками (6 из 12): отбор опускает её у травоядных с 1 до
~0.2, разнообразие падает, и они проигрывают хищникам. Без хищников живут все
12 миров.

С каннибализмом (8 сидов × 20 000 тиков, без хищников) не вымирает ни один мир,
но это гонка размеров: крупный ест мелких, и медианный размер растёт с 24 до
~80 (у отдельных — до 230), а травоядных остаётся 50‒180 вместо ~2000.
...
базовый мир 6000x4000. Темп растений, их потолок, стартовые популяции и миграция
хищников растут вместе с площадью, поэтому плотность — а с ней и баланс —
остаётся прежней. Масштаб — от 1 до 10 000.
...
Гены слоя под еду не подстраиваются: травоядные сами ищут, где выгоднее жить.
