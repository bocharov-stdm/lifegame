...
Эволюционная симуляция: растения и травоядные в двумерном мире. У травоядных
...
- **В игре**: пауза (Пробел), тик (→), скорость (+/−), колесо — зум к курсору,
  ...
  партии (и на паузе), I — окно «Статистика»: сытость, где живут травоядные и
  ...
  рядом со средним по миру. Подсадка травоядных кликом, «Заново» — та же
...
cargo run -p life-report --release -- --veg-mix 1 1                 # стратегии поровну на старте
...
В рассказе: итог; сколько травоядных съели сородичи и сколько умерло с голоду;
...
| `life-core/src/vegetarian/`, `plant.rs` | существа: фенотип, стратегии поведения |
...
девятую часть. Без хищников живут все 12 проверенных миров; мутагенность отбор
опускает примерно до 0.4.

С каннибализмом (8 сидов × 20 000 тиков) не вымирает ни один мир,
