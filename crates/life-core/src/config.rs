...
//!
//! Значения подобраны перебором через headless-прогоны. Критерии: травоядные
...
/// базового травоядного падает примерно до 70%: заметная экономия, но ищет
...
/// снят без него. Игра включает его сама.
